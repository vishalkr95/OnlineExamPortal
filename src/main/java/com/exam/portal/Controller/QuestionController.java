...
import com.exam.portal.Repository.codingQuestionRepository;
import com.exam.portal.Repository.testCaseRepository;
...
public class QuestionController{
    ...
    @Autowired
    codingQuestionRepository crepo;
    ...
    @PostMapping("/organiser/codingquestion/add")
    public String addCodingQuestion(@ModelAttribute codingQuestion codingQuestion ,@RequestParam(name="exam_id")Long exam_id,@RequestParam(name = "testcases[]") List<String> testcase,@RequestParam(name = "answer[]") List<String> answer){
        Exam exam=examRepo.findById(exam_id).get();
        
        codingQuestion.setExams(exam);
        crepo.save(codingQuestion);
        
        System.out.println(codingQuestion.getStatement());
        List<testCase> testcases =new ArrayList<>();
        
        for(int i=0;i<testcase.size();i++) {
        	testCase t=new testCase(codingQuestion, testcase.get(i),answer.get(i));
        	testcases.add(t);
        	trepo.save(t);
        }
        
        //codingQuestion.setTestCase(testcases);
