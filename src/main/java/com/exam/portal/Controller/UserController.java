...
public class UserController {
    ...
    @GetMapping(value = {"/{examCode}/section"})
    public String showSection(HttpSession session,@PathVariable(name = "examCode")String examCode, Model model) {
    	String redirectUrl="redirect:/"+examCode+"/login";
        try{
            if(checkValidExamCode(examCode)){
                if(isLoggedInForExam(session,examCode)){
    ...
    @GetMapping(value = {"/{examCode}/aptiexam", "/{examCode}/exam/{question_no}"})
    ...
    @GetMapping(value = {"/{examCode}/codingexam", "/{examCode}/codingexam/{question_no}"})
    public String showUserDashboardCoding(HttpSession session, @PathVariable(name = "examCode")String examCode, Model model,@PathVariable(name = "question_no",required = false)Integer question_no){
        String redirectUrl="redirect:/"+examCode+"/login";
        try{
            if(checkValidExamCode(examCode)){
                if(isLoggedInForExam(session,examCode)){
                    ...
                    List<codingQuestion> all_questions = exam.getCodingquestions();

                    if(question_no==null)
                        question_no=1;

                    codingQuestion currentQuestion = all_questions.get(question_no - 1);

                   
                    model.addAttribute("question",currentQuestion);
                    model.addAttribute("exam",exam);
                    model.addAttribute("question_no",question_no);
                    return "user/codingdashboard";
    ...
    public String saveAnswers(HttpSession session,@PathVariable(name = "examcode")String examcode,@RequestParam(name = "answer_id",required = false)Long answer_id,@RequestParam(name = "question_id")Long question_id){
        System.out.println("hello indasasjkdmnasmdm,sad,csmad,sadcmsamdas,d,sa");
    ...
    @PostMapping("{examcode}/submitCode")
    public String saveCodingAnswers(HttpSession session,@PathVariable(name = "examcode")String examcode,@RequestParam(name = "answer_id",required = false)Long answer_id,@RequestParam(name = "question_id")Long question_id){
        System.out.println("hello indasasjkdmnasmdm,sad,csmad,sadcmsamdas,d,sa");
        String redirectUrl="redirect:/"+examcode+"/codingexam";
        try{
            if(checkValidExamCode(examcode)){
                if(isLoggedInForExam(session,examcode)){
